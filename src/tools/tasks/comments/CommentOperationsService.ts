...
export class CommentOperationsService {
   ...
   */
  static async createComment(taskId: number, commentText: string): Promise<any> {
   ...
   */
  static async fetchTaskComments(taskId: number): Promise<any[]> {
    const client = await getClientFromContext();
   ...
   */
  static getCommentCount(comments: any[]): number {
